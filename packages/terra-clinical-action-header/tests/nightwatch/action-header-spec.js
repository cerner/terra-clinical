...
module.exports = resizeTo(['tiny', 'small', 'medium', 'large', 'huge', 'enormous'], {
  ...
  'Displays an action header with close button': (browser) => {
    ...
    // console.log(driver.getCapabilities().getCapability("chrome"));
    let width = screenWidth(browser);
    // = screenWidth(browser);
    browser.windowSize('width', (result) => {
      // eslint-disable-next-line no-console
      console.log(`The test breakpoint width is: ${width} and the broswer width is ${JSON.stringify(result)}`);
    });

    browser.perform((done) => {
      width = screenWidth(browser);
      console.log(width);
      done();
    });
    browser.perform((done) => {
    browser.resizeWindow(width, 789, () => console.log('resized'));

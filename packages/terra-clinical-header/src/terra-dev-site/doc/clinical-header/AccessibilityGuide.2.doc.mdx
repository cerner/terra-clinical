...
- Sectioning content can be labeled using a combination of the [`aria-labelledby`](https://www.w3.org/WAI/tutorials/page-structure/labels/#using-aria-labelledby) and `id` attributes, with the label concisely describing the purpose of the section. This technique is useful for situations where there is more than one sectioning element on the same page.
...
Terra Clinical Header **must** meet the following success criteria:
...
Apart from the above criteria the clinical header supports the following success criteria:

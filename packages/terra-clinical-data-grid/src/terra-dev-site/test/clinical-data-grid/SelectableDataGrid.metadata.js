...
export default {
  ...
  steps: [
    {
      name: 'tabs across selectable headers',
      action: () => {
        global.browser.keys(new Array(4).fill('Tab'));
      },
    },
    {
      name: 'tabs backwards across selectable headers',
      action: () => {
        global.browser.keys(['Shift'].concat(new Array(3).fill('Tab')).concat(['Shift']));
      },
    },
    {
      name: 'tabs across selectable sections and cells',
      action: () => {
        global.browser.keys(new Array(128).fill('Tab'));
      },
    },
    {
      name: 'tabs backwards selectable sections and cells',
      action: () => {
        global.browser.keys(['Shift'].concat(new Array(3).fill('Tab')).concat(['Shift']));
      },
    },
    {
      name: 'with cell selections',
      refreshUrl: true,
      action: () => {
        global.browser.click('[data-cell-label="section_0-0-Column-0"]');
      },
    },
    {
      name: 'with cell selection hover',
      refreshUrl: true,
      themeableProperties: {
        '--terra-clinical-data-grid-cell-hover-background-color': 'blue',
      },
      action: () => {
        global.browser.moveToObject('[data-accessibility-id="8"]');
      },
    },
    {
      name: 'with row selections',
      refreshUrl: true,
      action: () => {
        global.browser.click('#selections-example-Pinned-Row-Row-0-Section-section_0 > *:first-child');
      },
    },
    {
      name: 'with row selection hover',
      refreshUrl: true,
      themeableProperties: {
        '--terra-clinical-data-grid-row-hover-background-color': 'blue',
      },
      action: () => {
        global.browser.moveToObject('[data-accessibility-id="7"]');
      },
    },
    {
      name: 'with column header selections',
      refreshUrl: true,
      action: () => {
        global.browser.click('#selectable-data-grid [class*="HeaderCell"]:nth-child(2)');
      },
    },
  ],
};
